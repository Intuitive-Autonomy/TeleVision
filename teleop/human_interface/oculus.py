import time
import numpy as np
from human_interface.teleop_core import BaseTeleopInterface, TeleopAction, TeleopObservation
from utils.general_utils import run_threaded_command
from utils.transformations import quat_diff, quat_to_euler, rmat_to_quat
...
class OculusPolicy(BaseTeleopInterface):
    ...
    def _update_internal_state(self, num_wait_sec=5, hz=50):
        ...
        while True:
            ...
            for arm in ['left', 'right']:
                ...
                self._state[arm]["hand2head"] = poses[controller_id]
    ...
    def get_mujoco_action(self, hands_cartesian_position) -> TeleopAction:
        action = self.get_default_action()
        # add button data to action
        buttons = self._state["buttons"]
        action.extra['buttons'] = buttons
        # arm command
        for arm in ['right', 'left']:
            # TODO(kun): Support gripper position
            default_position = np.ones(7)
            # if hands_cartesian_position.get(arm) is None:
            #     print(f"Warning arm: {arm} is None")
            #     hands_cartesian_position[arm] = default_position
            # robot_obs = {'cartesian_position': hands_cartesian_position[arm], 'gripper_position': 0.0}
            # if self._state[arm]["poses"] is not None:
            #     action[arm] = self._calculate_action(robot_obs, arm)
            if self._state[arm]["hand2head"] is not None:
                xpos = self._state[arm]['hand2head'][:3, -1]
                quat = R.from_matrix(self._state[arm]['hand2head'][:3, :3]).as_quat()
                action[arm] = np.concatenate((xpos, quat))
        # base command
        base_action = np.zeros(3)
        if 'rightJS' in buttons:
            base_action[0] = buttons['rightJS'][1]
