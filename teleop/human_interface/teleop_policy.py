from human_interface import INTERFACE_MAP
...
class TeleopPolicy:
    def __init__(self, config: AttrDict) -> None:
        ...
        for part in self.controllers:
            if (self.controllers[part] is not None) and (self.controllers[part] not in self.interfaces):
                print(f'Initializing {self.controllers[part]} interface')
    ...
    def get_mujoco_action(self, hands_cartesian_position) -> TeleopAction:
        ...
        for interface in self.interfaces:
            interface_action[interface] = self.interfaces[interface].get_mujoco_action(hands_cartesian_position)
